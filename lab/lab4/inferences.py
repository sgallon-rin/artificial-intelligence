...
def enumeration_ask(X, e, bn):
    # X: query variable a string e.g. 'MaryCall'
    # e: evidence variable a dist e.g. {'MaryCall':True}
    ...
    Xvals = bn.variable_values(X)
    for val in Xvals:
        Q[val] = enumerate_all(bn.variables, extend(e, X, val), bn)
    return Q.normalize()
    # END_YOUR_CODE
...
def enumerate_all(variables, e, bn):
    ...
    if Y in e.keys():
        ...
        Yvals = bn.variable_values(Y)
        s = 0
        for val in Yvals:
            s += Ynode.p(val, e) * enumerate_all(rest, extend(e, Y, val), bn)
        return s
    # END_YOUR_CODE
...
def elimination_ask(X, e, bn, order=reversed):
    ...
    #raise NotImplementedError
    factors = []
    evidences = list(e.keys())
    #notHidden = list(e.keys())
    #notHidden.append(X)
    #notHidden.extend(X)
    #注意，X可能是一个字符串，也可能是一个列表，直接将判断加在条件里
    for var in order(bn.variables):
        factors.append(make_factor(var, e, bn)) 
        if var not in evidences and var not in X:
